...
export default function PostDetailPage() {
  ...
  const renderMedia = (media_urls: string[], media_type?: string) => {
    ...
    return (
      <Carousel className="w-full relative group mt-4">
          ...
          {media_urls.map((url, index) => {
            ...
            return (
              <CarouselItem key={index} className="pl-2 md:pl-4 basis-1/3">
                <div className="relative aspect-square bg-muted rounded-lg overflow-hidden shadow-lg border border-muted-foreground/10">
                  {currentIsImage ? (
                    <img src={url} alt={`Post media ${index + 1}`} className="w-full h-full object-cover" />
                  ) : currentIsVideo ? (
                    <video src={url} controls className="w-full h-full object-cover" />
                  ) : (
                    <a href={url} target="_blank" rel="noopener noreferrer" className="flex items-center justify-center p-4 text-primary hover:underline h-full">

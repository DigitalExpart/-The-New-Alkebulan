-- Fixed Products Backend Setup for Supabase
-- This script handles existing policies and fixes syntax errors

-- 1. Drop existing policies if they exist (to avoid conflicts)
DROP POLICY IF EXISTS "Users can view all active products" ON public.products;
DROP POLICY IF EXISTS "Users can view their own products" ON public.products;
DROP POLICY IF EXISTS "Users can create their own products" ON public.products;
DROP POLICY IF EXISTS "Users can update their own products" ON public.products;
DROP POLICY IF EXISTS "Users can delete their own products" ON public.products;

DROP POLICY IF EXISTS "Users can view variants of active products" ON public.product_variants;
DROP POLICY IF EXISTS "Users can manage variants of their own products" ON public.product_variants;

DROP POLICY IF EXISTS "Users can view images of active products" ON public.product_images;
DROP POLICY IF EXISTS "Users can manage images of their own products" ON public.product_images;

DROP POLICY IF EXISTS "Users can view videos of active products" ON public.product_videos;
DROP POLICY IF EXISTS "Users can manage videos of their own products" ON public.product_videos;

DROP POLICY IF EXISTS "Authenticated users can view categories" ON public.product_categories;
DROP POLICY IF EXISTS "Authenticated users can view subcategories" ON public.product_subcategories;

DROP POLICY IF EXISTS "Users can view inventory of active products" ON public.product_inventory;
DROP POLICY IF EXISTS "Users can manage inventory of their own products" ON public.product_inventory;

DROP POLICY IF EXISTS "Users can view reviews of active products" ON public.product_reviews;
DROP POLICY IF EXISTS "Users can create reviews for active products" ON public.product_reviews;
...
DROP POLICY IF EXISTS "Users can view their own favorites" ON public.product_favorites;
DROP POLICY IF EXISTS "Users can manage their own favorites" ON public.product_favorites;

DROP POLICY IF EXISTS "Users can view their own product views" ON public.product_views;
...
-- 3. Create product variants table (if not exists)
...
-- 5. Create product videos table (if not exists)
...
-- 6. Create product categories table (if not exists)
...
-- 7. Create product subcategories table (if not exists)
...
-- 8. Create product inventory table (if not exists)
...
-- 9. Create product reviews table (if not exists)
...
-- 10. Create product favorites table (if not exists)
...
-- 11. Create product views table (if not exists)
...
-- 12. Create indexes (if not exist)
CREATE INDEX IF NOT EXISTS idx_products_user_id ON public.products(user_id);
CREATE INDEX IF NOT EXISTS idx_products_category ON public.products(category);
CREATE INDEX IF NOT EXISTS idx_products_status ON public.products(status);
CREATE INDEX IF NOT EXISTS idx_products_created_at ON public.products(created_at);
CREATE INDEX IF NOT EXISTS idx_products_price_range ON public.products(actual_price, sales_price);
...
CREATE INDEX IF NOT EXISTS idx_product_inventory_variants ON public.product_inventory USING GIN(variant_combination);
...
-- 14. Create RLS policies (fresh creation)
...
CREATE POLICY "Users can manage variants of their own products" ON public.product_variants
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM public.products 
      WHERE id = product_id AND auth.uid() = user_id
    )
  );
...
-- Product categories and subcategories policies
...
-- 17. Insert default categories and subcategories (if not exist)
...
-- Get the category IDs and insert subcategories
DO $$
DECLARE
  physical_id UUID;
  digital_id UUID;

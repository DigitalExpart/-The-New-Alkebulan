-- Fixed Product Storage Setup for Supabase
-- This script fixes the timestamp parameter issue and sets up storage policies

-- 1. Drop existing storage policies if they exist
DROP POLICY IF EXISTS "Public can view product images" ON storage.objects;
DROP POLICY IF EXISTS "Product owners can upload images" ON storage.objects;
DROP POLICY IF EXISTS "Product owners can update images" ON storage.objects;
DROP POLICY IF EXISTS "Product owners can delete images" ON storage.objects;

DROP POLICY IF EXISTS "Public can view product videos" ON storage.objects;
DROP POLICY IF EXISTS "Product owners can upload videos" ON storage.objects;
DROP POLICY IF EXISTS "Product owners can update videos" ON storage.objects;
DROP POLICY IF EXISTS "Product owners can delete videos" ON storage.objects;
...
-- 4. Create function to generate unique file names (fixed timestamp parameter)
...
BEGIN
  RETURN user_id::TEXT || '_' || 
         EXTRACT(EPOCH FROM file_timestamp)::TEXT || '_' || 
         original_name;
END;
$$ LANGUAGE plpgsql;

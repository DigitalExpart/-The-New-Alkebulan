-- Comprehensive Products Backend Setup for Supabase
-- This script creates all necessary tables, storage, and policies for product management
...
-- 5. Create product categories table (for reference)
...
-- 6. Create product subcategories table (for reference)
...
-- 7. Create product inventory table (for tracking stock)
CREATE TABLE IF NOT EXISTS public.product_inventory (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  product_id UUID REFERENCES public.products(id) ON DELETE CASCADE NOT NULL,
  variant_combination JSONB, -- stores variant combination as key-value pairs
  quantity INTEGER NOT NULL DEFAULT 0,
  reserved_quantity INTEGER DEFAULT 0, -- for items in cart/checkout
...
CREATE TABLE IF NOT EXISTS public.product_favorites (
  ...
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  UNIQUE(product_id, user_id) -- one favorite per user per product
);

-- 10. Create product views table (for analytics)
...
-- Product categories and subcategories policies (read-only for all authenticated users)
...
-- Product views policies (for analytics)
...
-- 16. Insert default categories and subcategories
...
-- 17. Create storage buckets for media files
-- Note: These need to be created in the Supabase dashboard or via API
-- INSERT INTO storage.buckets (id, name, public) VALUES
--   ('product-images', 'product-images', true),
--   ('product-videos', 'product-videos', true);

-- 18. Create storage policies (these will be applied to the buckets created above)
-- Note: These policies need to be created after the buckets exist
